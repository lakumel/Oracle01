...
/*alter table 을 사용해서 제약 조건 생성, 수정, 삭제
        --Primary Key
        --Unique
        --Foreign Key
        --Check
...
/*Foreign Key 할당
        ...
        --FK가 적용된 컬럼과 , 부모테이블의 참조 컬럼의 자료형이 비슷하거나 같아야 한다.
        - FK 가 적용된 컬럼의 값은 부모테이블의 참조되는 컬럼의 값이 들어가 있어야 한다.
*/

--EMP01 테이블의

select * from dept01;       --부모 테이블 : d_dno
select * from emp01;        --자식 테이블 : dno   (FK)

--emp01의 dno 컬럼에 FK 할당.      부모 테이블 : d_dno
alter table emp01
add constraint FK_EMP01_DNO foreign key(dno) references DEPT01(D_DNO);

/*Unique 제약 조건 추가
    - 컬럼에 중복된 값이 없어야 한다.
    - null은 1번만 들어가 있어야 한다.
...
/*기존 테이블의 default
        --default 는 제약 조건이 아니다. 제약조건 이름을 가지지 않는다.
*/

select * from emp01;

--commission 컬럼에 값을 넣지 않을 경우 기본값(default) : 0
alter table emp01
modify commission default 0;

--default, null에 값을 넣을때 : default
insert into emp01
values(8000, '홍길동', '사무원', 7369, sysdate, 2000, default, 40);

--컬럼이 지정되지 않았을때 default 값이 들어감
