--부모 테이블은 다른테이블에서 참조하기 때문에 삭제가 불가능:cascade constraints
...
/*
    ...
    --number는 Oracle ,정수, 실수
    NUMBER(4): 정수 4자리만 입력
    NUMBER(10,2):실수,전체 10자리,소수점 이하 2자리
...
/* 2개 테이블 조인 */ 
-- Orders, members 테이블의 정보를 join  구문으로 출력 
...
-- 2개 테이블 조인 , orders , products 테이블 조인 
...
-- Outer JOIN 을 사용해서 구매( Oders )되지 않는 제품이 무엇인지 확인 : 
select * from orders; 
...
*/ 
-- 주문한 사용자 이름, 메일주소, 집주소, 구매한 제품, 가격, 구매 갯수, 구매 날짜를 출력 
select * from orders; 
select * from products;
select * from members;
select * from addresses; 
