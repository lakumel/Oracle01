...
--테이블 복사시 제약조건은 복사되지 않는다. -컬럼명, 자료형, 레코드(데이터) 복사됨
    --제약 조건은 복사되지 않는다.(Primary Key, Unique, Fofeign Key, Not Nullm Check)
    
    
-- 제약 조건 확인 : user_constraints <==데이터 사전 : 오라클에서 테이블의 정보를 담은 테이블
...
--EMP03 테이블의 ENO 컬럼에 Primary Key 할당. : 테이블에 1번만 할당, 컬럼에 중복된 데이터가 없어야 한다, null 없어야 한다.
desc emp03;
...
--EMP03 테이블의 DNO컬럼에 Foreign Key : 부모테이블 : DEPT03 DNO 컬럼을 참조
    ...
    --참조 컬럼의 자료형이 같은거나 비슷 해야 한다.
    -- DNO컬럼의 값이 부모테이블의 컬럼의 값이 참조된 값이 들어있어야 한다.
...
/*JOIN:DataBase 에는 많은 테이블이 존재합니다.      각 테이블은 다른 테이블과 관계(FK)를 가지고 있다.
    ...
    -- employee 테이블과 department 테이블을 하나의 테이블로 디자인 되었을 경우 부서정보가 계속 중복되어서 들어간다.
...
/*
    INNER 조인 : 두 테이블 모두 공통 키컬럼의 값이 일치하는 것만 출력   <==90%이상 사용
    select : 두 테이블의 컬럼을 명시,두 테이블의 공통 키컬럼을 출력시 테이블 이름을 명시
...
NATURAL JOIN : 오라클에서만 사용되는 JOIN,
    - 두 테이블의 공통 키컬럼을 Oracle이 자동으로 찾아서 JOIN
...
--SELF JOIN을 사용해서 사원의 직속 상관정보를 출력
